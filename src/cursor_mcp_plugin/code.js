...
\u{1F4A1} Tip: Only shapes and frames support stroke colors.`
  ...
  __name(createMultipleVariables, "createMultipleVariables");
  async function setMultipleVariableValues(params) {
    const { updates } = params;
    const commandId = generateCommandId();
    if (!updates || !Array.isArray(updates) || updates.length === 0) {
      throw new Error("Missing or invalid updates parameter");
    ...
    for (let chunkIndex = 0; chunkIndex < chunks.length; chunkIndex++) {
      ...
      const chunkPromises = chunk.map(async ({ variableId, modeId, value }) => {
        try {
          ...
          return { success: false, variableId, modeId, error: error.message };
        }
      });
      const chunkResults = await Promise.all(chunkPromises);
      chunkResults.forEach((result) => {
  ...
  __name(setMultipleVariableValues, "setMultipleVariableValues");
...
\u{1F4A1} Tip: Node must be inside a frame, group, or page.`
  ...
  async function createMultipleComponentInstances(params) {
  ...
  __name(createMultipleComponentInstances, "createMultipleComponentInstances");
  async function setMultipleComponentPropertyReferences(params) {
    const { bindings } = params;
    const commandId = generateCommandId();
    if (!bindings || !Array.isArray(bindings) || bindings.length === 0) {
      throw new Error("Missing or invalid bindings parameter");
  ...
  __name(setMultipleComponentPropertyReferences, "setMultipleComponentPropertyReferences");
  ...
  async function handleCommand(command, params) {
    switch (command) {
      ...
      case "set_multiple_variable_values":
        return await setMultipleVariableValues(params);
      ...
      case "create_multiple_component_instances":
        return await createMultipleComponentInstances(params);
      case "set_multiple_component_property_references":
        return await setMultipleComponentPropertyReferences(params);
  ...
  figma.ui.onmessage = async (msg) => {
    ...
    switch (msg.type) {
      ...
      case "notify":
        ...
        break;
      case "resize":
        figma.ui.resize((_a = msg.width) != null ? _a : 350, (_b = msg.height) != null ? _b : 450);
        break;
      case "close-plugin":
        figma.closePlugin();
        break;
